...
void handle_lookup_request(LookupMsg *msg) {
     ...
     ******************************************************/
     ...
     if (responsible) {
        ...
        // trimitem raspunsul inapoi la initiator
        ...
        // cautam cel mai apropiat finger
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < nr_lookups; ++i) {
        ...
        // trimitem mesajul de tip TAG_LOOKUP_REQ catre propriul rank
    ...
    int completed_lookups = 0; // lookup-uri locale intoarse
    int nodes_finished = 0; // noduri care au semnalat DONE
    int sent_done_signal = 0; // flag pentru a trimite DONE o singura data

    // Loop-ul ruleaza pana cand toate nodurile au anuntat ca au terminat
    while (nodes_finished < world_size) {
        MPI_Status status;
        LookupMsg msg;
        ...
        case TAG_LOOKUP_REP:
            // am primit raspuns de la un lookup initiat de mine
        ...
        // verificam daca am terminat treaba locala si nu am anuntat inca
        if (!sent_done_signal && completed_lookups == nr_lookups) {
            // trimitem DONE la toate nodurile pentru a incrementa nodes_finished
            for (int r = 0; r < world_size; ++r) {
